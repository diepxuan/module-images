...
class Gd2 extends AbstractAdapter
{
     ...
     *
     * @throws FileSystemException|\OverflowException
     */
    public function open($filename): void
    {
        ...
        if (!$filename || 0 === filesize($filename) || !$this->validateURLScheme($filename)) {
     ...
     */
    public function resize($frameWidth = null, $frameHeight = null): void
    {
     ...
     * @SuppressWarnings(PHPMD.UnusedFormalParameter)
     */
    public function watermark($imagePath, $positionX = 0, $positionY = 0, $opacity = 30, $tile = false): void
    {
    ...
    public function crop($top = 0, $left = 0, $right = 0, $bottom = 0)
    {
        ...
        if (IMAGETYPE_PNG === $this->_fileType) {
    ...
    {
        if (false !== stripos($memoryValue, 'G')) {
            return (int) $memoryValue * 1_024 ** 3;
        }
        if (false !== stripos($memoryValue, 'M')) {
            return (int) $memoryValue * 1_024 * 1_024;
        }
        if (false !== stripos($memoryValue, 'K')) {
            return (int) $memoryValue * 1_024;
        }
     ...
     * @param string $text
     */
    protected function _createImageFromText($text): void
    {
     ...
     */
    protected function _createImageFromTtfText($text, $font): void
    {
     ...
     * @param int $height
     */
    protected function _createEmptyImage($width, $height): void
    {
    ...
    private function validateURLScheme(string $filename): bool
    {
        ...
        if ($url && isset($url['scheme']) && !\in_array($url['scheme'], $allowed_schemes, true)) {
            return false;
        }
    ...
    private function fillBackgroundColor(&$imageResourceTo): void
    {
        ...
        [$red, $green, $blue] = $this->_backgroundColor ?: [0, 0, 0];
    ...
    private function applyTransparency(&$imageResourceTo, $transparentIndex): void
    {
        ...
        if ($transparentIndex >= 0 && $transparentIndex < imagecolorstotal($this->_imageHandler)) {
            try {
                ...
                [$red, $green, $blue] = array_values($colorsForIndex);
    ...
    private function createWatermarkBasedOnPosition(
        ...
        if ($this->getWatermarkWidth()
            ...
            && self::POSITION_STRETCH !== $this->getWatermarkPosition()
        ) {
        ...
        if (self::POSITION_TILE === $this->getWatermarkPosition()) {
            $tile = true;
        } elseif (self::POSITION_STRETCH === $this->getWatermarkPosition()) {
            $watermark = $this->createWaterMark($watermark, $this->_imageSrcWidth, $this->_imageSrcHeight);
        } elseif (self::POSITION_CENTER === $this->getWatermarkPosition()) {
        ...
        } elseif (self::POSITION_TOP_RIGHT === $this->getWatermarkPosition()) {
        ...
        } elseif (self::POSITION_TOP_LEFT === $this->getWatermarkPosition()) {
            $this->imagecopymergeWithAlphaFix(
                $this->_imageHandler,
                $watermark,
                $positionX,
        ...
        } elseif (self::POSITION_BOTTOM_RIGHT === $this->getWatermarkPosition()) {
            $positionX = $this->_imageSrcWidth - imagesx($watermark);
        ...
        } elseif (self::POSITION_BOTTOM_LEFT === $this->getWatermarkPosition()) {
            $positionY = $this->_imageSrcHeight - imagesy($watermark);
            $this->imagecopymergeWithAlphaFix(
                $this->_imageHandler,
                $watermark,
     ...
     */
    private function imageDestroy(): void
    {
     ...
     */
    private function _saveAlpha($imageHandler): void
    {
